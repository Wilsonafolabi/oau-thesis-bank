    ...
    <div className="min-h-screen bg-[#F8F9FA] font-sans text-slate-900 flex">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-[280px] bg-white border-r border-slate-200 flex flex-col transition-transform duration-200 md:sticky md:top-0 md:h-screen md:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-3 border-t border-slate-200 shrink-0">
          ...
          <button onClick={() => go('landing')} className="flex items-center w-full px-3 py-2.5 text-sm font-medium text-red-600 rounded-xl hover:bg-red-50 mt-1"><LogOut className="w-4 h-4 mr-3" /> Sign out</button>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white/95 backdrop-blur border-b border-slate-200 h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            {!admin && <button onClick={() => go('admin-dashboard')} title="Open admin portal" className="hidden lg:flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50"><Shield className="h-3.5 w-3.5" /> Admin</button>}
